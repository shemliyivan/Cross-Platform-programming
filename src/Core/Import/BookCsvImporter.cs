...
// Роздільник — крапка з комою: не конфліктує з комою в назвах товарів.
    ...
    private static ParseOutcome ParseLine(string line)
    {
        ...
        [_, _, _, var yearStr] when !int.TryParse(yearStr, out int year) || year < 0 
        => new ParseFailed($"рік '{yearStr}' не є коректним числом"),

        [var id, var isbn, var title, var yearStr] 
        => new ParseOk(new ProductDto(id, isbn, title, int.Parse(yearStr))),
